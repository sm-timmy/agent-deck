...
> Fork Diff vs upstream `v1.4.0`
>
> This fork is optimized for a `Ghostty + tmux + Codex` setup.
>
> What's changed:
> - `tmux` attach path is switched back to native `tmux attach-session`, which preserves pane history on reattach and avoids the `history=0` regression seen with the PTY wrapper.
> - Codex session detection treats `CODEX_SESSION_ID` from tmux environment as authoritative, so live sessions do not regress to stale transcript IDs.
> - Codex previews prefer transcript-backed assistant output when tmux scrollback is empty after attach/detach, so the latest response still shows up in Agent Deck.
> - The risky global stdin/key-sequence translator is intentionally not reintroduced. Upstream `v1.4.0` already includes the Kitty keyboard compatibility fix, and the old wrapper had broken arrows/layout in real use.
>
> Operational target:
> - stable reattach behavior for Codex sessions
> - preserved tmux history
> - better last-response visibility in the TUI
> - no extra input-layer hacks beyond upstream `v1.4.0`
