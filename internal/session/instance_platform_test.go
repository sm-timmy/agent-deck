...
// TestUpdateCodexSession_EnvAuthoritative verifies that CODEX_SESSION_ID from tmux
// wins over any later probe logic in UpdateCodexSession.
func TestUpdateCodexSession_EnvAuthoritative(t *testing.T) {
	skipIfNoTmuxServer(t)

	const tmuxSessionName = "agentdeck_test_codex_env_authoritative"
	...
	const envID = "019d6710-b3bb-7121-aca4-3bf1cc955529"
	if err := exec.Command("tmux", "set-environment", "-t", tmuxSessionName, "CODEX_SESSION_ID", envID).Run(); err != nil {
		t.Fatalf("tmux set-environment failed: %v", err)
	}

	inst := &Instance{
		ID:              "test-codex-env-authoritative",
		Title:           "test-codex-env-authoritative",
		Tool:            "codex",
		CodexSessionID:  "old-stale-id",
		CodexDetectedAt: time.Time{},
		tmuxSession:     tmux.ReconnectSession(tmuxSessionName, "codex-env-authoritative", "/tmp", ""),
	}

	inst.UpdateCodexSession(nil)

	if inst.CodexSessionID != envID {
		t.Errorf("CodexSessionID = %q, want %q (tmux env must be authoritative)", inst.CodexSessionID, envID)
	}
	if inst.CodexDetectedAt.IsZero() {
		t.Error("CodexDetectedAt should be set after update from tmux env")
	}
}
